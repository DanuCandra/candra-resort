...
@section('content')
    <section id="page-top" class="page-hero" style="background-image:url('{{ $content?->image_path ? Storage::url($content->image_path) : asset('landing-lage/img/hero/hero-3.jpg') }}')"><div class="container text-center"><h1>{{ $content?->title ?? 'Promosi' }}</h1><p>{{ $content?->content ?? 'Lebih banyak pengalaman dengan penawaran terbaik.' }}</p></div></section>
